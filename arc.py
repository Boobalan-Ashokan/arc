...
# Function to perform classification using Lazy Predict
def perform_classification(df, target_column):
    ...
    # Perform classification using LazyClassifier
...
def display_classification_results(models, predictions):
    st.subheader("Classification Models and Accuracy Scores")
...
if uploaded_file is not None:
    tab1, tab2, tab3, tab4,tab5= st.tabs(["Home", "Data Analysis", "Multivariate Analysis", "Regression Analysis","Models"])
    ...
    with tab5:
        tabc, tabr= st.tabs(["Classification", "Regression"])
        with tabc:
            ...
            if not df.empty:
                ...
                if st.button("Perform Classification"):
                    with st.spinner('Processing'):
                        models, predictions = perform_classification(df, target_column)
                    display_classification_results(models, predictions)
        with tabr:
            st.title("Regression")
            if not df.empty:
                ...
                if st.button("Perform Regression"):
                    with st.spinner('Processing'):
                        models, predictions = perform_regression(df, target_column)
                    display_classification_results(models, predictions)
            
else:
    st.title("ARC")
    st.write("uncover patterns and relationships in your data")
    st.subheader("Please upload the file for Analysis")